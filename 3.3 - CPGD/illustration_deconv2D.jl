...
sigma = 0.1 * 2pi
phi(x,y) = exp(-(x^2 + y^2)/(2*sigma^2))
phi_derx(x,y) = - x / (sigma^2) * phi(x,y)
phi_dery(x,y) = - y / (sigma^2) * phi(x,y)
...
cont = contourf(xs,ys,sum(r_obs[i] * phi.(xs' .- θ_obs[i,1],ys .- θ_obs[i,2]) for i=1:m0), 100);
...
axis([0,2π,0,2π]);
savefig("fig/mdpi_cpgd_fourier_2D.pdf",bbox_inches="tight")
...
# axis([0,2π,0,2π]);
# # legend()
# savefig("fig/mdpi_cpgd_fourier_2D_finer_mesh.pdf",bbox_inches="tight")
